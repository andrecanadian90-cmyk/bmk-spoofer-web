...
// Download asset binary from Roblox with Place Context Bypass
export async function downloadAsset(assetId, cookie = null) {
  const cleanCookie = getCleanCookie(cookie);
  
  // Use a high-privilege game Place ID context to convince Roblox 
  // that the request is made by a running server engine loading the asset.
  const bypassPlaceId = '185655149'; // Adopt Me (or any major active game)
  ...
  // 1. Try fetching via assetdelivery with location parsing
  ...
  if (infoRes.ok) {
    ...
    if (contentType.includes('application/json')) {
      ...
      if (buffer.length > 0) return { buffer, size: buffer.length, contentType };
    }
  }

  // 2. Try newer assetdelivery API v2
  if (!downloadUrl) {
  ...
  // 3. Fallback to direct asset server URL with Place ID context parameters (This is the primary bypass method)
  if (!downloadUrl) {
    downloadUrl = `https://assetdelivery.roblox.com/v1/asset/?id=${assetId}&serverplaceid=${bypassPlaceId}`;
  }

  // Download binary data from the resolved/fallback URL
  ...
  if (!dlRes.ok) {
    // If the standard CDN endpoints fail with 403, try direct roblox data endpoint as a final bypass fallback
    const directUrl = `https://data.roblox.com/Data/Upload.ashx?assetId=${assetId}`;
    ...
    if (directRes.ok) {
      ...
      if (buffer.length > 0) {
        return { buffer, size: buffer.length, contentType: 'application/octet-stream' };
      }
    }
