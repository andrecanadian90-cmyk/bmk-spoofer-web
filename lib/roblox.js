...
// This is the same endpoint Roblox Studio Animation Editor uses when publishing
async function uploadAnimationLegacy(cookie, name, buffer) {
  ...
  // Primary: Studio Animation Editor publish endpoint
  const url = `https://www.roblox.com/ide/publish/uploadnewanimation?assetid=0&name=${encodeURIComponent(cleanName)}&description=&ispublic=False&allowComments=False`;
  ...
  if (res.ok) {
    ...
    // Response is plain text asset ID
    const assetId = text.match(/^\d+$/) ? text : null;
    if (assetId) return { assetId, operationId: null };
  }

  // Fallback: legacy data.roblox.com upload endpoint
  const csrf2 = await getCsrfToken(cookie);
  const url2 = `https://data.roblox.com/Data/Upload.ashx?assetTypeId=24&name=${encodeURIComponent(cleanName)}&description=&ispublic=False&allowComments=False`;
  ...
  if (res2.ok) {
    ...
    const assetId2 = text2.match(/^\d+$/) ? text2 : null;
    if (assetId2) return { assetId: assetId2, operationId: null };
  }

  throw new Error(`Legacy animation upload failed (${res.status} / ${res2.status})`);
}

// Upload asset to Roblox — animations use legacy Studio endpoint, others use Open Cloud
export async function uploadAsset(apiKey, creatorId, assetType, name, description, buffer, cookie = null) {
  // Animations MUST use legacy endpoint — Open Cloud creates "Model" type, not "Animation"
  if (assetType === 'Animation' && cookie) {
    return await uploadAnimationLegacy(cookie, name, buffer);
  }

  // Open Cloud for non-animation assets (Model, Audio, Decal, etc.)
