...
// Download asset binary from Roblox — multi-endpoint cascade
export async function downloadAsset(assetId, cookie = null) {
  const cleanCookie = getCleanCookie(cookie);
  const bypassPlaceId = '185655149';
  ...
  // Build base headers mimicking the real Roblox game client
  const gameClientHeaders = {
    'User-Agent': 'Roblox/WinInet',
    'Requester': 'Client',
    'Roblox-Place-Id': bypassPlaceId,
    'Accept': '*/*',
  ...
  // Build headers mimicking Roblox Studio
  const studioHeaders = {
  ...
  // Endpoint cascade — try each until one succeeds
  const endpoints = [
    // 1. Game client style: /v1/asset/ with full query params
    {
      url: `https://assetdelivery.roblox.com/v1/asset/?id=${assetId}&expectedAssetType=Animation&skipSigningScripts=1&clientInsert=1&placeId=${bypassPlaceId}&accept-encoding=gzip`,
      headers: gameClientHeaders,
      parse: 'binary',
    },
    // 2. Legacy www endpoint (different permission model)
    {
      url: `https://www.roblox.com/asset/?id=${assetId}`,
      headers: studioHeaders,
      parse: 'binary',
    },
    // 3. assetgame subdomain (older CDN path)
    {
      url: `https://assetgame.roblox.com/asset/?id=${assetId}`,
      headers: studioHeaders,
      parse: 'binary',
    },
    // 4. v1 JSON metadata → follow location
    {
      url: `https://assetdelivery.roblox.com/v1/assetId/${assetId}`,
      headers: { ...gameClientHeaders, Accept: 'application/json' },
      parse: 'json-location',
    },
    // 5. v2 JSON metadata → follow locations array
    {
      url: `https://assetdelivery.roblox.com/v2/assetId/${assetId}`,
      headers: { ...gameClientHeaders, Accept: 'application/json' },
      parse: 'json-locations',
    },
    // 6. serverplaceid context param
    {
      url: `https://assetdelivery.roblox.com/v1/asset/?id=${assetId}&serverplaceid=${bypassPlaceId}`,
      headers: gameClientHeaders,
      parse: 'binary',
    },
  ];
  ...
  for (const ep of endpoints) {
    try {
      const res = await fetchWithRetry(ep.url, {
        headers: ep.headers,
        redirect: 'follow',
      }, 1); // single attempt per endpoint, don't waste retries
      ...
      } else if (ep.parse === 'json-location') {
        const ct = res.headers.get('content-type') || '';
        if (ct.includes('json')) {
          const info = await res.json();
          const loc = info.location || info.Location;
          if (loc) {
            const dlRes = await fetchWithRetry(loc, { headers: gameClientHeaders, redirect: 'follow' }, 2);
            if (dlRes.ok) {
      ...
      } else if (ep.parse === 'json-locations') {
        const data = await res.json();
  ...
  throw new Error(`Failed to download asset ${assetId}: all endpoints returned 403/error — asset is creator-locked`);
}
