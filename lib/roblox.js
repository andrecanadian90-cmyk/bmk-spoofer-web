    ...
    if (creatorType === 'User' && placeIds.length === 0) {
      try {
        ...
        if (groupsRes.ok) {
          ...
          const groups = (groupsData.data || []).slice(0, 50);
          
          const groupFetchPromises = groups.map(async (g) => {
            try {
              const gGamesRes = await fetch(`https://games.roblox.com/v2/groups/${g.group.id}/games?sortOrder=Asc&limit=10`, {
                headers: { Accept: 'application/json' },
...
async function downloadAssetInternal(assetId, cookie = null) {
  ...
  let detailReason = '';
  if (debugLogs.some(log => log.includes('403'))) {
    detailReason = ` Aset ini dikunci/privat oleh pembuatnya (${info.creator || 'unknown'}) atau otorisasi Cookie ditolak oleh Roblox.`;
  }
