...
async function resolveCreatorPlaceIds(assetId, cookie) {
  ...
  try {
    ...
    // Step 4: If user has no direct places, search creator's groups' games
    if (creatorType === 'User' && placeIds.length === 0) {
      try {
        ...
        if (groupsRes.ok) {
          ...
          for (const g of (groupsData.data || []).slice(0, 15)) {
            const gGamesRes = await fetch(`https://games.roblox.com/v2/groups/${g.group.id}/games?sortOrder=Asc&limit=20`, {
              headers: { Accept: 'application/json' },
              signal: AbortSignal.timeout(5000),
            });
            if (gGamesRes.ok) {
              const gGamesData = await gGamesRes.json();
              for (const game of (gGamesData.data || [])) {
                const placeId = game.rootPlace?.id;
                if (placeId) placeIds.push(String(placeId));
              }
            }
