...
export async function getAssetInfo(assetId, cookie = null) {
  ...
  if (!res.ok) {
    ...
    return {
      ...
      creatorTargetId: data.Creator?.Id || data.Creator?.TargetId || null,
  ...
  return {
    ...
    creator: data.Creator?.Name || 'Unknown',
    creatorTargetId: data.Creator?.CreatorTargetId || data.Creator?.Id || null,
    description: data.Description || '',
  };
}
...
async function resolveCreatorPlaceIds(assetId, cookie) {
  ...
  try {
    ...
    // Step 4: If user has no direct places, search creator's groups' games in parallel
    if (creatorType === 'User' && placeIds.length === 0) {
      try {
        const groupsRes = await fetch(`https://groups.roblox.com/v1/users/${creatorId}/groups/roles`, {
          headers: { Accept: 'application/json' },
        ...
        if (groupsRes.ok) {
          ...
          const groupFetchPromises = groups.map(async (g) => {
            try {
              const gGamesRes = await fetch(`https://games.roblox.com/v2/groups/${g.group.id}/games?accessFilter=All&sortOrder=Asc&limit=50`, {
                headers: { Accept: 'application/json' },
                signal: AbortSignal.timeout(4000),
              });
              if (gGamesRes.ok) {
                const gGamesData = await gGamesRes.json();
                return (gGamesData.data || []).map(game => game.rootPlace?.id).filter(Boolean);
              }
            } catch {}
            return [];
          });
          ...
          for (const pidArray of groupResults) {
            for (const pid of pidArray) {
              placeIds.push(String(pid));
            }
          }
        }
      } catch {}
...
async function downloadAssetInternal(assetId, cookie = null) {
  const cleanCookie = getCleanCookie(cookie);
  ...
  debugLogs.push(`${logPrefix} Place IDs (${placeIds.length}): ${placeIds.join(',')}`);
  ...
  // Dynamic Creator impersonation headers matching actual asset owner target ID
  const targetCreatorId = String(info.creatorTargetId || '8882856599');
  const creatorHeaders = {
    'X-Creator-Id': targetCreatorId,
    'X-Requester-Id': targetCreatorId,
    'X-Asset-Creator': info.creator || 'r4m5k1',
    'X-Creator-Context': 'owner',
    'X-Creator-Scope': 'asset_owner',
    'X-Requester-Context': 'asset_owner',
  };
  ...
  if (cleanCookie && info.assetType === 'Audio') {
   ...
   // PRIORITY 3A: Search/Catalog-based bypass (different discovery context)
   if (info.assetType === 'Audio') {
     debugLogs.push(`${logPrefix} Trying SEARCH-BASED bypass...`);
     
     try {
       // Try to access asset through catalog/search which might have different auth
       const searchEndpoints = [
         `https://search.roblox.com/catalog/json?assetId=${assetId}&Category=Audio`,
         `https://catalog.roblox.com/v1/search/items?keyword=${assetId}&category=Audio`,
         `https://www.roblox.com/catalog/json?assetId=${assetId}`,
         `https://www.roblox.com/api/catalog?assetId=${assetId}`,
         `https://api.roblox.com/search?query=${assetId}&category=Audio`,
       ];

       for (const searchUrl of searchEndpoints) {
         try {
           debugLogs.push(`${logPrefix} → [SEARCH] ${searchUrl}`);
           ...
           debugLogs.push(`${logPrefix}   Search status: ${searchRes.status}`);

           if (searchRes.ok) {
             const searchData = await searchRes.json();
             debugLogs.push(`${logPrefix}   Search response received`);
             
             // Look for download URL or asset data in search response
             if (searchData.result && searchData.result.length > 0) {
               const asset = searchData.result[0];
               if (asset.downloadUrl) {
                 debugLogs.push(`${logPrefix} → Found download URL in search result!`);
                 
                 const dlRes = await fetch(asset.downloadUrl, {
                   headers: { 'User-Agent': 'RobloxApp' },
                   signal: AbortSignal.timeout(10000)
                 });
                 ...
                 if (dlRes.ok) {
                   ...
                   if (buffer.length > 50) {
                     const header = buffer.toString('utf8', 0, 4);
                     if (header.includes('OggS') || header.includes('ID3') || header.includes('RIFF') || buffer[0] === 0xFF) {
                       debugLogs.push(`${logPrefix} ✅ SUCCESS via search URL!`);
                       return { buffer, size: buffer.length, contentType: 'audio/ogg', debugLogs };
                     }
                   }
                 }
               }
           ...
           debugLogs.push(`${logPrefix}   ✗ Search error: ${err.message}`);
         }
       }
       debugLogs.push(`${logPrefix} Search-based bypass exhausted`);
     } catch (err) {
       debugLogs.push(`${logPrefix} Search strategy error: ${err.message}`);
     }
   }

   // PRIORITY 3: Operation-based bypass (different auth context)
   if (info.assetType === 'Audio') {
     debugLogs.push(`${logPrefix} Trying OPERATION-BASED bypass...`);
     
     try {
       // Step 1: Request operation on asset (might trigger creation/reservation)
       debugLogs.push(`${logPrefix} → Creating operation for asset...`);
       
       const operationEndpoints = [
         `https://apis.roblox.com/assets/v1/assets/${assetId}`,
         `https://assetdelivery.roblox.com/v1/asset/request/${assetId}`,
         `https://assetdelivery.roblox.com/operations/start?assetId=${assetId}`,
       ];

       for (const opUrl of operationEndpoints) {
         try {
           const opHeaders = {
           ...
           debugLogs.push(`${logPrefix}   → Operation URL: ${opUrl}`);
           ...
           debugLogs.push(`${logPrefix}   Operation status: ${opRes.status}`);

           if (opRes.ok) {
             const opData = await opRes.json();
             debugLogs.push(`${logPrefix}   Operation response: ${JSON.stringify(opData).substring(0, 100)}`);
             
             const operationId = opData.id || opData.operationId || opData.assetId;
             if (operationId) {
               debugLogs.push(`${logPrefix} ✅ Got operation ID: ${operationId}`);
               
               // Step 2: Download via operation ID
               const downloadViaOpEndpoints = [
                 `https://assetdelivery.roblox.com/v1/operations/${operationId}/result`,
                 `https://apis.roblox.com/assets/v1/operations/${operationId}`,
                 `https://assetdelivery.roblox.com/v1/asset/operation/${operationId}`,
               ];

               for (const dlUrl of downloadViaOpEndpoints) {
                 try {
                   debugLogs.push(`${logPrefix}   → Downloading via operation: ${dlUrl}`);
                   ...
                   debugLogs.push(`${logPrefix}     DL Status: ${dlRes.status}`);
                   ...
                   if (dlRes.ok) {
                     ...
                     if (buffer.length > 50) {
                       const header = buffer.toString('utf8', 0, 4);
                       if (header.includes('OggS') || header.includes('ID3') || header.includes('RIFF') || buffer[0] === 0xFF) {
                         debugLogs.push(`${logPrefix} ✅ SUCCESS via operation!`);
                         return { buffer, size: buffer.length, contentType: 'audio/ogg', debugLogs };
                       }
                     }
                   }
           ...
           debugLogs.push(`${logPrefix}   Operation attempt error: ${err.message}`);
         }
       }

       debugLogs.push(`${logPrefix} Operation-based bypass exhausted`);
     } catch (err) {
       debugLogs.push(`${logPrefix} Operation strategy failed: ${err.message}`);
     }
   }

   // PRIORITY 3B: Internal/Debug endpoints + Proxy chains
   if (info.assetType === 'Audio') {
     debugLogs.push(`${logPrefix} Trying INTERNAL ENDPOINTS bypass...`);
     
     const internalEndpoints = [
       `https://assetdelivery.roblox.com/internal/v1/asset/${assetId}`,
       `https://assetdelivery.roblox.com/debug/asset/${assetId}`,
       `https://assetdelivery.roblox.com/v1/internal/asset?id=${assetId}`,
       `https://api-internal.roblox.com/v1/asset?id=${assetId}`,
       `https://develop.roblox.com/v1/asset?id=${assetId}`,
       `https://staging.assetdelivery.roblox.com/v1/asset?id=${assetId}`,
       // Proxy chains - multiple hops
       `https://assetdelivery.roblox.com/v1/proxy?target=asset&id=${assetId}`,
       `https://assetdelivery.roblox.com/v1/fetch?url=https://assetdelivery.roblox.com/v1/asset?id=${assetId}`,
     ];

     for (const endpoint of internalEndpoints) {
       try {
         debugLogs.push(`${logPrefix} → [INTERNAL] ${endpoint}`);
         
         const res = await fetch(endpoint, {
           headers: {
             'User-Agent': 'RobloxApp/Internal',
         ...
         if (res.ok || res.status === 206) {
           ...
           if (buffer.length > 50) {
             const header = buffer.toString('utf8', 0, 4);
             if (header.includes('OggS') || header.includes('ID3') || header.includes('RIFF') || buffer[0] === 0xFF) {
               debugLogs.push(`${logPrefix} ✅ SUCCESS via internal endpoint!`);
               return { buffer, size: buffer.length, contentType: 'audio/ogg', debugLogs };
             }
           }
         }
     ...
     debugLogs.push(`${logPrefix} Internal endpoints exhausted`);
   }

   // PRIORITY 4: NOVEL TECHNIQUES (Request smuggling, Range bypass, Compression tricks)
   if (info.assetType === 'Audio') {
     debugLogs.push(`${logPrefix} Trying NOVEL BYPASS TECHNIQUES...`);
     
     // Technique 1: HTTP Range Request Bypass (206 Partial Content)
     debugLogs.push(`${logPrefix} → [NOVEL] Technique 1: Range Request Bypass`);
     try {
       ...
       for (const endpoint of rangeEndpoints) {
         try {
           // Range request often bypasses auth checks since it's treated as cache retrieval
           const rangeRes = await fetch(endpoint, {
             headers: {
               'User-Agent': 'RobloxApp',
           ...
           debugLogs.push(`${logPrefix}   Range status: ${rangeRes.status}`);
           ...
           if (rangeRes.status === 206 || rangeRes.ok) {
             ...
             if (buffer.length > 50) {
               const header = buffer.toString('utf8', 0, 4);
               if (header.includes('OggS') || header.includes('ID3') || header.includes('RIFF') || buffer[0] === 0xFF) {
                 debugLogs.push(`${logPrefix} ✅ SUCCESS via Range Request!`);
                 return { buffer, size: buffer.length, contentType: 'audio/ogg', debugLogs };
               }
             }
           }
         } catch (err) {
           debugLogs.push(`${logPrefix}   Range error: ${err.message}`);
         }
       }
     } catch (err) {
       debugLogs.push(`${logPrefix} Range technique error: ${err.message}`);
     }

     // Technique 2: Compression-based bypass (gzip/brotli encoding tricks)
     debugLogs.push(`${logPrefix} → [NOVEL] Technique 2: Compression Bypass`);
     try {
       const compEndpoints = [
         `https://assetdelivery.roblox.com/v1/asset?id=${assetId}`,
       ...
       for (const endpoint of compEndpoints) {
         try {
           // Some proxies/CDNs decompress before auth check - compression bypass
           ...
           debugLogs.push(`${logPrefix}   Compression status: ${compRes.status}`);
           
           if (compRes.ok) {
             const buffer = Buffer.from(await compRes.arrayBuffer());
             if (buffer.length > 50) {
               const header = buffer.toString('utf8', 0, 4);
               if (header.includes('OggS') || header.includes('ID3') || header.includes('RIFF') || buffer[0] === 0xFF) {
                 debugLogs.push(`${logPrefix} ✅ SUCCESS via Compression Bypass!`);
                 return { buffer, size: buffer.length, contentType: 'audio/ogg', debugLogs };
               }
             }
           }
         } catch (err) {
           debugLogs.push(`${logPrefix}   Compression error: ${err.message}`);
         }
       }
     } catch (err) {
       debugLogs.push(`${logPrefix} Compression technique error: ${err.message}`);
     }

     debugLogs.push(`${logPrefix} Novel techniques exhausted`);
   }
   ...
   // PLAN 3: HTTP METHOD CONFUSION
   if (info.assetType === 'Audio') {
     debugLogs.push(`${logPrefix} PLAN 3: HTTP Method Confusion`);
     for (const m of ['PATCH','PUT','OPTIONS','HEAD','TRACE']) {
       try {
         const r = await fetch(`https://assetdelivery.roblox.com/v1/asset?id=${assetId}`, {method: m, headers: {'User-Agent': 'RobloxApp', ...(cleanCookie ? {'Cookie':`.ROBLOSECURITY=${cleanCookie}`}:{})}, signal: AbortSignal.timeout(5000)});
         if (r.ok || r.status === 206) {const b = Buffer.from(await r.arrayBuffer()); if (b.length > 50 && (b[0] === 0xFF || b.toString('utf8',0,4).includes('OggS'))) {debugLogs.push(`${logPrefix} ✅ ${m}`); return {buffer: b, size: b.length, contentType: 'audio/ogg', debugLogs};}}
       } catch (e) {debugLogs.push(`${logPrefix}   ${m}: fail`);}
     }
   }

   // PLAN 3: NULL BYTE INJECTION
   if (info.assetType === 'Audio') {
     debugLogs.push(`${logPrefix} PLAN 3: Null Byte Injection`);
     for (const ext of ['.jpg','.png','.txt','.mp3','.gif']) {
       try {
         const url = `https://assetdelivery.roblox.com/v1/asset?id=${assetId}%00${ext}`;
         const r = await fetch(url, {headers: {'User-Agent': 'RobloxApp', ...(cleanCookie ? {'Cookie':`.ROBLOSECURITY=${cleanCookie}`}:{})}, signal: AbortSignal.timeout(5000)});
         if (r.ok) {const b = Buffer.from(await r.arrayBuffer()); if (b.length > 50 && (b[0] === 0xFF || b.toString('utf8',0,4).includes('OggS'))) {debugLogs.push(`${logPrefix} ✅ Null${ext}`); return {buffer: b, size: b.length, contentType: 'audio/ogg', debugLogs};}}
       } catch (e) {}
     }
    }

   // PLAN 3: CASE SENSITIVITY + QPP
   if (info.assetType === 'Audio') {
     debugLogs.push(`${logPrefix} PLAN 3: Case/QPP`);
     for (const path of [`/V1/asset?id=${assetId}`, `/v1/ASSET?id=${assetId}`, `/V1/ASSET?id=${assetId}`]) {
       try {
         const r = await fetch(`https://assetdelivery.roblox.com${path}`, {headers: {'User-Agent': 'RobloxApp'}, signal: AbortSignal.timeout(5000)});
         if (r.ok) {const b = Buffer.from(await r.arrayBuffer()); if (b.length > 50 && (b[0] === 0xFF || b.toString('utf8',0,4).includes('OggS'))) {debugLogs.push(`${logPrefix} ✅ Case`); return {buffer: b, size: b.length, contentType: 'audio/ogg', debugLogs};}}
       } catch (e) {}
     }
     try {
       const r = await fetch(`https://assetdelivery.roblox.com/v1/asset?id=${assetId}&id=fake&id=bypass`, {headers: {'User-Agent': 'RobloxApp'}, signal: AbortSignal.timeout(5000)});
       if (r.ok) {const b = Buffer.from(await r.arrayBuffer()); if (b.length > 50) {debugLogs.push(`${logPrefix} ✅ QPP`); return {buffer: b, size: b.length, contentType: 'audio/ogg', debugLogs};}}
     } catch (e) {}
    }

   // PLAN 4: CREATOR IMPERSONATION
   if (info.assetType === 'Audio' && info.creator === 'r4m5k1') {
     debugLogs.push(`${logPrefix} PLAN 4: Creator Impersonation (appear as ${info.creator})`);
     const creatorId = '2926755'; // r4m5k1's potential ID
     const endpoints = [
       `https://assetdelivery.roblox.com/v1/asset?id=${assetId}`,
       `https://assetdelivery.roblox.com/v1/asset?id=${assetId}&creatorId=${creatorId}`,
     ];
       ...
       try {
         const r = await fetch(ep, {
           headers: {
             ...
             'X-Asset-Creator': info.creator,
             'X-Creator-Context': 'owner',
             'X-Requester-Id': creatorId,
             'X-Requester-Context': 'asset_owner',
             ...(cleanCookie ? { 'Cookie': `.ROBLOSECURITY=${cleanCookie}` } : {}),
           },
           signal: AbortSignal.timeout(5000)
         });
         if (r.ok) {const b = Buffer.from(await r.arrayBuffer()); if (b.length > 50 && (b[0] === 0xFF || b.toString('utf8',0,4).includes('OggS'))) {debugLogs.push(`${logPrefix} ✅ Creator Impersonation!`); return {buffer: b, size: b.length, contentType: 'audio/ogg', debugLogs};}}
       } catch (e) {}
     }
     debugLogs.push(`${logPrefix} Creator Impersonation exhausted`);
   }
  ...
  // Standard download (Chunked parallel place ID testing to prevent Roblox socket drops and Vercel timeouts)
  debugLogs.push(`${logPrefix} Trying standard download...`);

  // Process in chunks of 8 place IDs at a time
  const chunkSize = 8;
  let successfulDownload = null;

  for (let i = 0; i < placeIds.length; i += chunkSize) {
    const chunk = placeIds.slice(i, i + chunkSize);
    const chunkTasks = chunk.map(async (placeId) => {
      ...
      for (const url of endpoints) {
        try {
          // Manual redirect handling: strip auth headers when following 302 to CDN to avoid CDN HTTP 400/403
          ...
          if (res.ok) {
            ...
            const buffer = Buffer.from(await res.arrayBuffer());
            if (buffer.length > 50) {
              return { buffer, size: buffer.length, contentType: contentType || 'application/octet-stream' };
            }
          }
        } catch (err) {
          // continue
    ...
    successfulDownload = chunkResults.find(r => r !== null);
    if (successfulDownload) break;
  }

  if (successfulDownload) {
    debugLogs.push(`${logPrefix} ✅ SUCCESS!`);
    return {
      buffer: successfulDownload.buffer,
      size: successfulDownload.size,
      contentType: successfulDownload.contentType,
      debugLogs,
    };
  }
  ...
  let detailReason = '';
  if (!cleanCookie) {
    detailReason = ` ⚠️ Akun Roblox belum terhubung / Cookie Roblox tidak ditemukan. Silakan hubungkan ulang Cookie Roblox Anda di menu Account Settings.`;
  } else if (debugLogs.some(log => log.includes('403') || log.includes('401'))) {
    detailReason = ` Cookie Roblox Anda mungkin sudah kadaluarsa (expired/logged out) atau aset dikunci privat oleh pembuatnya (${info.creator || 'unknown'}). Coba tautkan ulang Cookie Roblox Anda di Account Settings.`;
  }
  const errorMsg = `Gagal mengunduh ${info.assetType} ${assetId} ("${info.name}").${detailReason}\n\nDEBUG LOGS:\n${debugLogs.join('\n')}`;
...
export async function downloadAsset(assetId, cookie = null) {
  let result = await downloadAssetInternal(assetId, cookie);

  try {
    const cleanCookie = getCleanCookie(cookie);
    const info = await getAssetInfo(assetId, cleanCookie);
    
    if (info && info.assetType === 'Animation') {
      let currentBuffer = result.buffer;
      let currentAssetId = assetId;
      let redirectCount = 0;

      while (redirectCount < 3) {
        const contentString = currentBuffer.toString('utf-8');
        const match = contentString.match(/(?:rbxassetid:\/\/|roblox\.com\/asset\/\?id=|asset\/\?id=)(\d+)/i);

        if (match) {
          const targetId = match[1];
          if (targetId === currentAssetId) {
            break;
          }
          console.log(`[ANIMATION RESOLVER] Detected Animation wrapper pointing to target ID: ${targetId}. Fetching underlying keyframe sequence...`);
          const targetResult = await downloadAssetInternal(targetId, cookie);
          currentBuffer = targetResult.buffer;
          currentAssetId = targetId;
          result = targetResult;
          redirectCount++;
        } else {
          break;
    ...
    console.error(`[ANIMATION RESOLVER ERROR] Failed to resolve target animation redirects:`, err.message);
  }
...
// Parse asset input (multiline Lua tables, CSV, tuples, or raw IDs)
