...
export async function downloadAsset(assetId, cookie = null) {
  ...
  // Pre-verification: check if asset exists at all to throw accurate error
  let info = null;
  try {
  ...
  // Step 1: Resolve the creator's Place IDs for context spoofing
  const creatorPlaceIds = await resolveCreatorPlaceIds(assetId, cleanCookie);

  // Build ordered list of Place IDs to try as context
  const placeIds = [...creatorPlaceIds];
  if (!placeIds.includes('185655149')) {
  ...
  if (info && info.assetType === 'Audio') {
    throw new Error(`Gagal mengunduh audio ${assetId}. Audio ini diset PRIVAT oleh pembuatnya (${info.creator}). Roblox membatasi akses download audio privat milik orang lain.`);
  }
