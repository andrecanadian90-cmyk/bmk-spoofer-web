...
// Download asset binary from Roblox — Creator Place Context Bypass + Audio Bypass
export async function downloadAsset(assetId, cookie = null) {
  ...
  // Audio-specific bypass attempts
          ...
          for (const url of endpoints) {
            try {
              ...
              if (res.ok) {
                ...
                // Skip JSON responses (operation IDs, not actual data)
                if (contentType.includes('json')) continue;
                
                const buffer = Buffer.from(await res.arrayBuffer());
                
                // Validate it's actual audio data (not empty, not error response)
                if (buffer.length > 100 && !buffer.toString('utf8', 0, 10).includes('{')) {
                  return { 
                    buffer, 
                    size: buffer.length, 
  ...
  // Standard download attempts for all asset types
  for (const placeId of placeIds) {
    ...
    for (const url of endpoints) {
      try {
        ...
        if (res.ok) {
          ...
          const buffer = Buffer.from(await res.arrayBuffer());
          if (buffer.length > 0 && !buffer.toString('utf8', 0, 10).includes('{')) {
            return { 
              buffer, 
              size: buffer.length, 
  ...
  throw new Error(`Gagal mengunduh ${info.assetType} ${assetId}. Coba lagi atau hubungi support jika masalah berlanjut.`);
}
