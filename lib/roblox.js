...
export async function downloadAsset(assetId, cookie = null) {
  ...
  try {
    ...
    console.log(`${logPrefix} Asset Info:`, { type: info.assetType, name: info.name, creator: info.creator });
  } catch (err) {
  ...
  if (cleanCookie) {
    ...
    console.log(`${logPrefix} CSRF Token obtained`);
  }
  ...
  console.log(`${logPrefix} Place IDs to try:`, placeIds);

  // Prioritize authenticated headers when cookie available
  const baseHeaders = {
  ...
  console.log(`${logPrefix} Auth available:`, !!cleanCookie);

  // If authenticated, try authenticated endpoints first
  if (cleanCookie && info.assetType === 'Audio') {
    console.log(`${logPrefix} Trying authenticated audio endpoints...`);
    ...
    for (const url of authAudioEndpoints) {
      try {
        console.log(`${logPrefix} → Trying: ${url}`);
        ...
        console.log(`${logPrefix}   Status: ${res.status}, ContentType: ${res.headers.get('content-type')}`);

        if (res.ok) {
          const contentType = res.headers.get('content-type') || '';
          
          // Skip JSON-like responses
          if (contentType.includes('json') || contentType.includes('text/')) {
            console.log(`${logPrefix}   ✗ Skipped (JSON/text response)`);
            continue;
          }
          
          const buffer = Buffer.from(await res.arrayBuffer());
          console.log(`${logPrefix}   Buffer size: ${buffer.length} bytes`);
          
          // Check if valid binary (not JSON, not text error)
          if (buffer.length > 50) {
            const firstBytes = buffer.toString('utf8', 0, Math.min(50, buffer.length));
            console.log(`${logPrefix}   First bytes: ${firstBytes.substring(0, 20).replace(/\n/g, '\\n')}`);
            
            if (!firstBytes.includes('{') && !firstBytes.includes('<!') && !firstBytes.includes('error')) {
              console.log(`${logPrefix} ✅ SUCCESS from authenticated endpoint!`);
              return { 
                buffer, 
                size: buffer.length, 
              ...
              console.log(`${logPrefix}   ✗ Failed validation (JSON/HTML/error detected)`);
            }
          } else {
            console.log(`${logPrefix}   ✗ Buffer too small`);
          }
        }
      } catch (err) {
    ...
    console.log(`${logPrefix} All authenticated endpoints failed`);
  }
  ...
  // Audio-specific multi-strategy bypass
  if (info.assetType === 'Audio') {
    console.log(`${logPrefix} Trying multi-UA + Place context strategy (${userAgents.length}x${referers.length}x${placeIds.length} combinations)...`);
    let attemptCount = 0;
            ...
            try {
              ...
              if (res.ok) {
                ...
                if (buffer.length > 50) {
                  ...
                  // OGG: 'OggS', MP3: 'ID3' or 0xFF, WAV: 'RIFF'
                  if (header.includes('OggS') || header.includes('ID3') || header.includes('RIFF') || buffer[0] === 0xFF) {
                    console.log(`${logPrefix} ✅ SUCCESS on attempt ${attemptCount}!`);
                    return { 
                      buffer, 
                      size: buffer.length, 
                      contentType: contentType || 'audio/ogg' 
    ...
    console.log(`${logPrefix} All ${attemptCount} multi-strategy attempts exhausted`);
  }

  // Standard download for other asset types
  console.log(`${logPrefix} Trying standard download for ${info.assetType}...`);
  for (const placeId of placeIds) {
    ...
    for (const url of endpoints) {
      try {
        ...
        if (res.ok) {
          ...
          const buffer = Buffer.from(await res.arrayBuffer());
          if (buffer.length > 0) {
            console.log(`${logPrefix} ✅ SUCCESS on standard endpoint!`);
            return { 
              buffer, 
              size: buffer.length, 
  ...
  console.log(`${logPrefix} ❌ ALL ATTEMPTS FAILED`);
  throw new Error(`Gagal mengunduh ${info.assetType} ${assetId}. Asset mungkin dilindungi atau tidak accessible.`);
}
