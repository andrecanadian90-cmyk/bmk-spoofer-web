...
export async function getAssetInfo(assetId) {
  ...
  if (!res.ok) {
    ...
    const fallback = await fetch(`https://api.roblox.com/marketplace/productinfo?assetId=${assetId}`);
...
export async function downloadAsset(assetId) {
  ...
  const infoRes = await fetch(`https://assetdelivery.roblox.com/v1/assetId/${assetId}`, {
    headers: {
  ...
  if (!downloadUrl) {
    const v2Res = await fetch(`https://assetdelivery.roblox.com/v2/assetId/${assetId}`, {
      headers: {
        Accept: 'application/json',
        'Roblox-Place-Id': '0',
        'User-Agent': 'Roblox/WinInet',
...
export async function uploadAsset(apiKey, creatorId, assetType, name, description, buffer) {
  // Open Cloud supports: Model, Decal, Audio
  // For animations, we try Animation first, fallback to Model
  const uploadType = assetType === 'Animation' ? 'Model' : (assetType || 'Model');
  // Note: Open Cloud may not directly support "Animation" type upload.
  // Animations are uploaded as Model containers. When the model contains
  // a KeyframeSequence, Roblox recognizes it as an animation.
  ...
  // Response can be: { assetId, operationId } or { path: "operations/xxx", done, response: { assetId } }
...
export function parseAssetInput(input) {
  ...
  for (const line of lines) {
    // LUA table: { "Name", 12345 } or {"Name", 12345}
    ...
    // URL format: roblox.com/catalog/12345
